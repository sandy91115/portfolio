...
export function HeroScrub({
  ...
  const [ready, setReady] = useState(useTech3D);
  const [framesOk, setFramesOk] = useState(true);
  const [aspect, setAspect] = useState<number>(defaultAspect);
  const [scrollProgress, setScrollProgress] = useState<number>(0);
  const reduced = usePrefersReducedMotion();

  // If using image frames sequence (legacy mode)
  useEffect(() => {
    if (useTech3D || reduced || !frameUrl) {
      setReady(true);
      return;
    }
    let cancelled = false;
  ...
  }, [useTech3D, reduced, frameCount, frameUrl]);
  ...
  useEffect(() => {
    ...
    return () => ctx.revert();
  }, [ready, framesOk, reduced, aspect, frameCount, useTech3D]);
  ...
  return (
    ...
    >
      ...
      >
        {/* Sleek Deep Carbon Background with Ambient Tech Glow */}
        <div ref={bgRef} aria-hidden className="absolute inset-0 z-0 bg-[#06070a]" />
        <div
          aria-hidden
          className="absolute inset-0 z-0"
          style={{
            background: `radial-gradient(ellipse at 50% 40%, ${accentHex}25 0%, rgba(6,7,10,0.98) 72%)`,
          }}
        />
        <div
          aria-hidden
        ...
        <div className="relative z-10 flex h-full w-full flex-col items-center justify-center gap-3 md:gap-4">
          <h2
            ref={titleTopRef}
            aria-hidden
            className="font-black uppercase tracking-tight text-white drop-shadow-[0_10px_35px_rgba(0,0,0,0.8)]"
            style={{
          ...
          <div
            ref={cardRef}
            className="relative overflow-hidden rounded-[16px] shadow-[0_25px_90px_rgba(0,0,0,0.85)] ring-1 ring-white/15 will-change-transform md:rounded-[24px] bg-[#090b10]"
            style={{
          ...
          >
            <div aria-hidden="true" className="pointer-events-none absolute inset-0 z-20 shadow-[inset_0_0_120px_rgba(0,0,0,0.65)]" />
